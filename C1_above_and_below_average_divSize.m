...
exptArray = 11:14; % use corresponding dataIndex values
...
for e = 1:length(exptArray)
    ...
    for condition = 1:length(bubbletime)
        ...
        if isempty(traits_final) == 1
            ...
            clear assignedBins_raw raw_vals pdf_divSize_raw color
            ...
            population_GR = mean(traits_final(:,7));
            ...
            mean(traits_above(:,7)/population_GR), mean(traits_below(:,7)/population_GR);
            ...
            std(traits_above(:,7)/population_GR), std(traits_below(:,7)/population_GR);

            ];
    

            figure(condition+1)
            bar(trait_comparison)
            hold on
            errorbar([0.85,1.15; 1.85,2.15; 2.85,3.15; 3.85,4.15; 4.85,5.15],trait_comparison,trait_comparison_error,'.')
            title(strcat('traits of subpopulations: condition-',num2str(condition)))
